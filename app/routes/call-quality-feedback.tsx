...
import { AnalyticsSimpleCallFeedback, getDb, Meetings, Transcripts } from 'schema'
...
export const loader = async ({ request, context }: LoaderFunctionArgs) => {
	try {
		...
		if (!db) {
			return json({ meeting: null, participants: [], error: 'Database binding missing' })
		}
		...
		return json({ meeting: null, participants: [], error: e.message || String(e) })
	}
}
...
function formatDuration(startStr: string, endStr: string | null) {
	const normalizeDate = (s: string) => s.includes('T') || s.endsWith('Z') ? s : s.replace(' ', 'T') + 'Z';
		...
		return (
			<div className="min-h-[100dvh] bg-zinc-950 flex items-center justify-center p-6">
				<div className="bg-zinc-900 border border-white/10 rounded-2xl p-6 max-w-xs w-full text-center">
					<Icon type="ExclamationCircleIcon" className="w-10 h-10 text-red-500 mx-auto mb-3" />
					<h1 className="text-lg font-bold text-zinc-100 mb-1">Summary Unavailable</h1>
					<p className="text-zinc-500 text-xs mb-5">{error || 'Meeting not found'}</p>
					<Link to="/" className="inline-block w-full bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold py-2.5 rounded-xl transition-all">
						Return Home
					</Link>
				</div>
			</div>
		...
		<div className="min-h-[100dvh] bg-zinc-950 text-zinc-100 selection:bg-orange-500/30 font-sans">
			...
			<div className="relative max-w-xl mx-auto px-6 py-12 md:py-16">
				<div className="text-center mb-10">
					...
					<h1 className="text-3xl font-black tracking-tight mb-2">Meeting Over</h1>
					<p className="text-zinc-500 text-sm font-medium">Session details and assets</p>
				</div>

				{/* Compact Stats Row */}
				<div className="flex flex-col sm:flex-row gap-3 mb-6">
					<div className="flex-1 bg-zinc-900/50 backdrop-blur-md border border-white/5 p-4 rounded-2xl">
						<p className="text-[10px] font-bold text-orange-500 uppercase tracking-widest mb-1">Room</p>
						<p className="text-sm font-bold truncate">{meeting?.roomName || 'Private'}</p>
					</div>
					<div className="flex-1 bg-zinc-900/50 backdrop-blur-md border border-white/5 p-4 rounded-2xl">
						<p className="text-[10px] font-bold text-orange-500 uppercase tracking-widest mb-1">Duration</p>
						<p className="text-sm font-bold">
							{meeting ? formatDuration(meeting.created, meeting.ended) : '--'}
						</p>
					</div>
					<div className="flex-1 bg-zinc-900/50 backdrop-blur-md border border-white/5 p-4 rounded-2xl">
						<p className="text-[10px] font-bold text-orange-500 uppercase tracking-widest mb-1">Users</p>
				...
				<div className="space-y-4">
					...
					{participants.length > 0 && (
						<div className="bg-zinc-900/30 border border-white/5 p-5 rounded-2xl">
							<h2 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4">Participants</h2>
							<div className="flex flex-wrap gap-1.5">
								{participants.map((p, i) => (
									<div key={p.userId || i} className="bg-white/5 px-3 py-1 rounded-lg text-xs font-medium border border-white/5 hover:bg-white/10 transition-colors">
										{p.userName}
									</div>
								))}
							</div>
					...
					<div className="group relative bg-orange-500 rounded-2xl p-6 overflow-hidden transition-all active:scale-[0.99] border border-orange-400/20 shadow-lg shadow-orange-500/10">
						...
						<div className="absolute -top-4 -right-4 p-4 opacity-10 group-hover:scale-110 transition-transform">
							<Icon type="ArrowDownOnSquareIcon" className="w-20 h-24 text-white" />
						</div>
						<div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
							<div className="max-w-[240px]">
								<h2 className="text-lg font-black text-white mb-1 uppercase tracking-tight">Full Transcript</h2>
								<p className="text-orange-100 text-[11px] font-medium leading-tight">Complete record of the meeting conversation is ready for download.</p>
							</div>
							<button 
								onClick={() => window.open(`/api/transcript/${meetingId}`, '_blank')}
