	...
	return (
		<div className="flex items-start gap-4">
			<div className={`mt-0.5 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
				isJoined ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'
			}`}>
...
export function useUserJoinLeaveToasts(users: User[]) {
	...
	useEffect(() => {
		...
		newUsers.forEach((u) => {
			...
			dispatchToast(<UserJoinedOrLeftToast user={u} type="joined" />, { id: `join-${u.id}` })
		})

		usersLeft.forEach((u) => {
			if (u.id === 'ai') return
			dispatchToast(<UserJoinedOrLeftToast user={u} type="left" />, { id: `left-${u.id}` })
		})
